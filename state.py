...
class State:
    def __init__(
        ...
        # Initialize a valid 2D grid if state_representation is not provided
        self.state_representation = (
            state_representation if state_representation else self._initialize_empty_ship(8, 12)
        )
        self.depth = depth
    ...
    def _initialize_empty_ship(self, num_rows: int, num_cols: int):
        ...
        Each cell is initialized to None.
        """
        return [[None for _ in range(num_cols)] for _ in range(num_rows)]

    def init_start_state(self, manifest):
        """
        Initializes the starting state of the ship based on the provided manifest.

        Args:
            manifest (list): List of Container objects with positions.

        Returns:
            State: The current state after initializing.
        """
        ...
        for container in manifest:
            ...
            print(f"Placing container: {container}, Position: {position}")
            if not position or len(position) != 2:
                raise ValueError(f"Invalid position {position} for container {container}")
    ...
    def print_state_representation(self):
        """
        Prints the state representation in a grid format.
        """
        if not isinstance(self.state_representation, list):
            print("Error: state_representation is not a valid 2D list!")
            return
    ...
    def get_state_representation(self):
        ...
        """
        if not isinstance(self.state_representation, list):
            raise TypeError("state_representation must be a 2D list")
        return [row.copy() for row in self.state_representation]

    def __lt__(self, other):
        """
        Less-than comparator for use in priority queues (heapq).
        Compares based on the time attribute.
        """
...
#         # self.state_representation = [[Container() for _ in range(12)] for _ in range(8)]
#         self.depth = depth
#         self.last_moved_container = last_moved_container if last_moved_container is not None else [None, None]
#         self.time = time
#         self.parent_state = parent_state
